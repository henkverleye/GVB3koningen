...
try
{
	...
	if ((0 + Date("m")) < 8)
		--$currentJaar;
	$query = "SELECT sp_file_context.titel, sp_file_uploads.url, DATE_FORMAT(sp_file_context.begindatum, '%d-%m-%Y') FROM sp_file_uploads INNER JOIN sp_file_context ON sp_file_uploads.upload_key = sp_file_context.upload_key INNER JOIN sp_timestamp ON sp_file_uploads.timestamp_key = sp_timestamp.timestamp_key WHERE ((sp_file_context.schooljaar = $currentJaar) AND (sp_file_context.context = 'brief') AND (sp_file_context.begindatum <= now()) AND (sp_file_context.einddatum >= now())) ORDER BY sp_timestamp.timestamp DESC";
